process annovar {
   ...
   diff=\$((\$(grep "Start" ${project_id}_annovar.hg38_multianno.txt | wc -w)-\$(grep "QUAL" ${project_id}_annovar.hg38_multianno.vcf | wc -w)))
     
   sed -i "s/^Chr.*\$/\$(echo -e "\$(grep "Start" ${project_id}_annovar.hg38_multianno.txt | cut -d\$'\t' -f1-\$diff)\t\$(grep "QUAL" ${project_id}_annovar.hg38_multianno.vcf)")/" ${project_id}_annovar.hg38_multianno.txt
    """
}
