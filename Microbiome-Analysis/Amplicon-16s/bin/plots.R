...
rare_plot = ggrare(obj1,
  ...
  label="dpw",
  color="group")
ggsave(paste(argv$o,"/RarefactionCurve.png",sep=""), plot = rare_plot)
...
bp=plot_richness(obj1, x="group", color="dpw", measures="Chao1") +
  ...
  outlier.stroke=1,
  outlier.alpha=0.8)
ggsave(paste(argv$o,"/ChaosRichness.png",sep=""), plot = bp)
...
#tree
 tree=plot_tree(obj1,
   ...
   min.abundance=20) +
   coord_polar(theta="y")
ggsave(paste(argv$o,"/PhyloTree.png",sep=""), plot = tree)
...
ggsave(paste(argv$o,"/StackedBar.png",sep=""), plot = stacked_bar)
